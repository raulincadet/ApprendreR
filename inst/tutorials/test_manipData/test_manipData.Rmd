...
tab_retard<-cred%>%
  filter(Jours>30)
grade_result(
  pass_if(~(dim(.result)[1]==dim(tab_retard)[1])&   &(sum(.result)==sum(tab_retard$Credit)),"Félicitations!")
...
par30b<-cred>%>%
  filter(Jours>30)
grade_result(
  pass_if(~identical(.result==par30),"Félicitations!")||
    ~identical(.result==sum(par30b$Credit)),"Félicitations!")
